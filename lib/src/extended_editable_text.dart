...
class ExtendedEditableTextState extends State<ExtendedEditableText>
  ...
  void didUpdateWidget(ExtendedEditableText oldWidget) {
    ...
    if (widget.scrollController != null && widget.scrollController != oldWidget.scrollController) {
  ...
  TextEditingValue _handleSpecialTextSpan(TextEditingValue value) {
    if (supportSpecialText) {
      ...
      if (textChanged) {
        final TextSpan newTextSpan = widget.specialTextSpanBuilder.build(value.text, textStyle: widget.style);

        final TextSpan oldTextSpan = widget.specialTextSpanBuilder.build(_value.text, textStyle: widget.style);
  ...
  void updateFloatingCursor(RawFloatingCursorPoint point) {
    switch (point.state) {
      case FloatingCursorDragState.Start:
        ...
        TextPosition currentTextPosition = TextPosition(offset: renderEditable.selection.baseOffset);
        //zmt
        if (supportSpecialText) {
          currentTextPosition =
...
//        _selectionOverlay.showHandles();
  ...
  void bringIntoView(TextPosition position) {
    if (supportSpecialText) {
      position = convertTextInputPostionToTextPainterPostion(renderEditable.text, position);
    }
